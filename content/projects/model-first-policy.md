...
Most policy fights that look like values fights aren't. They're model fights, disagreements about how the world works, what causes what, whose experience counts as evidence, dressed up in ideology because we don't have better tools. People who could be acting together spend their energy arguing past each other, running incompatible mental simulations of the same intervention, each convinced the other is naive or captured.

The standard response is better messaging. This is almost always wrong. Agents don't update their behavior when you change the incentive structure. They update their *model*, and then behavior follows. An intervention aimed at behavior that ignores the model the agent is running will be reinterpreted, explained away, or absorbed without changing anything, sometimes while appearing to succeed in the short run.
...
The [MVP spec is public on GitHub](https://github.com/elbowspeak/model-first-policy). Development is in a private repository; if you want to contribute, request access there.
